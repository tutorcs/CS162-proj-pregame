...
void test_main(void) {
  ...
  CHECK(inumber(root_fd) != inumber(a_fd0), "\"/\" and \"/a\" must have different inumbers");
  ...
  if (remove("/a")) {
    ...
    a_fd1 = wrap_open("/a");
    a_fd2 = wrap_open(".");
    CHECK(inumber(a_fd1) == inumber(a_fd2), "\"/a\" and \".\" must have same inumber");
    CHECK(inumber(root_fd) != inumber(a_fd1), "\"/\" and \"/a\" must have different inumbers");
